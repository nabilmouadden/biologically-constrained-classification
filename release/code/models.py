...
def _build_dinov2_vitb14() -> nn.Module:
    ...
    # DINOv2 ViT-B/14 is the backbone-agnostic ablation control (only reached with
    # --backbone dinov2_vitb14; the default dinobloom_b needs none of this). Point
    # $WEIGHTS at a directory holding dinov2_vitb14.pth.
    env_w = os.environ.get("WEIGHTS")
    if not env_w:
        raise FileNotFoundError(
            "The dinov2_vitb14 ablation needs $WEIGHTS set to a directory containing "
            "dinov2_vitb14.pth (download from facebookresearch/dinov2). Not required "
            "for the default dinobloom_b backbone.")
