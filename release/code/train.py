...
# HF/torch caches default to ~/.cache; override via $HF_HOME etc. if you want a
# different location. To run on a cluster compute node with no internet, first
# populate the cache online, then export TRANSFORMERS_OFFLINE=1 / HF_HUB_OFFLINE=1
# yourself. We do NOT force offline mode here so DinoBloom-B can be fetched from
# HF-hub on first use on a fresh machine.
...
def main():
    ...
    ap.add_argument("--data_csv", required=True,
                    help="annotations.csv (header: filename,path,<class1>..<class7>); "
                         "GR-Neutro is not redistributed — supply your own image root")
    ap.add_argument("--data_root", required=True, help="image root directory")
