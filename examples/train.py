...
def train_epoch(model, loader, optimizer, loss_fn, device, mc_samples_train=5):
    """Train for one epoch"""
    model.train()
    epoch_loss = 0
    loss_components = {
    ...
    for batch_idx, (images, targets) in enumerate(progress_bar):
        ...
        # Forward pass
        outputs = model(images, training=True, mc_samples=mc_samples_train)
...
def validate(model, loader, loss_fn, device, mc_samples_val=50):
    """Validate the model"""
    model.eval()
    val_loss = 0
    loss_components = {
    ...
    with torch.no_grad():
        for images, targets in progress_bar:
            ...
            # Forward pass
            outputs = model(images, training=False, mc_samples=mc_samples_val)
...
def main():
    ...
    model_config = {
        ...
        'mc_samples_train': config['training'].get('mc_samples_train', 5),
    }
    ...
    # Only include backbone params if not frozen
    ...
    optimizer = optim.AdamW(param_groups, weight_decay=config['training'].get('weight_decay', 0.01))
    ...
    # MC samples from config (defaults match original hardcoded values)
    mc_samples_train = config['training'].get('mc_samples_train', 5)
    mc_samples_val = config['training'].get('mc_samples_val', 50)
    ...
    for epoch in range(config['training']['epochs']):
        ...
        # Train
        train_loss, train_loss_components = train_epoch(model, train_loader, optimizer, loss_fn, device, mc_samples_train)
        train_losses.append(train_loss)
        
        # Validate
        val_metrics = validate(model, val_loader, loss_fn, device, mc_samples_val)
