...
def parse_args():
    ...
    parser.add_argument('--mc_samples', type=int, default=None, help='Number of Monte Carlo samples (overrides config)')
...
def main():
    ...
    test_loader = DataLoader(
        ...
        shuffle=False,
        num_workers=config.get('inference', {}).get('num_workers', config['training'].get('num_workers', 4))
    )
    
    # MC samples: CLI arg overrides config, which defaults to 50
    mc_samples = args.mc_samples or config.get('inference', {}).get('mc_samples', 50)

    # Run inference
    print(f"Running inference with {mc_samples} Monte Carlo samples...")
    results = run_inference(model, test_loader, device, mc_samples=mc_samples)
