...
def predictive_entropy(mc_probs):
    ...
    return -mean_probs * torch.log(mean_probs + EPS)
...
def mutual_information(mc_probs):
    ...
    # Average entropy
    sample_entropies = -mc_probs * torch.log(mc_probs + EPS)  # [M, B, K]
    avg_sample_entropy = torch.mean(torch.sum(sample_entropies, dim=2), dim=0)  # [B]
    
    # Entropy of average probabilities
    entropy_avg_probs = -torch.sum(mean_probs * torch.log(mean_probs + EPS), dim=1)  # [B]
...
def compute_ood_scores(in_domain_probs, out_domain_probs):
    ...
    # Predictive entropy
    in_entropy = -np.sum(in_domain_probs * np.log(in_domain_probs + EPS), axis=1)
    out_entropy = -np.sum(out_domain_probs * np.log(out_domain_probs + EPS), axis=1)
