...
Inputs (cached on Ruche; SLURM only):
  --features  outputs/dinobloom_features.npz   (frozen) or a fine-tuned npz
  --manifest  outputs/p3_pilot/cell_manifest_full_extended.json  (labels)
  --morpho    outputs/morphometry_concepts/morphometry_concepts.csv  (measured concepts)
  --backbone_json  outputs/max_classification/<tag>.json (optional ft W-F1 reference)

Concept set: 10 measured concepts (vacuolization removed per concept_config v3).
...
# 10 measured concepts (vacuolization dropped per concept_config_gr_neutro.json v3).
CONCEPTS = [
...
def load_aligned(features_npz, manifest_json, morpho_csv):
...
def main():
    ...
    t0 = time.time()
    X, C, y, class_names, align = load_aligned(args.features, args.manifest, args.morpho)
