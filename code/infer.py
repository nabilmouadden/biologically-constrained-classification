...
* The checkpoints were trained with 11 concept-adapter outputs (the historical
  concept set, vacuolization included). The loader reads the concept list from
  the checkpoint, so the model rebuilds at the correct width automatically. The
  deterministic-morphometry block reports the canonical 10 concepts
  (vacuolization dropped per ``concepts_10.json``); see ``release/README.md``.
"""
...
# Canonical 10 measured concepts (vacuolization dropped). The deterministic
# morphometry actually computes 11; we surface these 10 as the released set.
...
def main():
    ...
    print(f"[infer] architecture mode={mode}  ({len(concepts)} concept-adapter outputs)")
