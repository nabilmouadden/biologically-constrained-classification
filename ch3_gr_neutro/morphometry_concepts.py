...
def compute_concepts(rgb: np.ndarray, seg: dict):
    ...
    # ---- chromatin: condensation & clumping (within nucleus) ---- #
    if nuc.sum() > 30:
        nvals = h_chan[nuc]
        # condensation: mean hematoxylin intensity (denser chromatin = darker)
        cond = np.mean(nvals)
        raw["chromatin_mean_H"] = float(cond)
        out["chromatin_condensation_level"] = _norm(cond, 0.0, 0.6)
    ...
    if cyto.sum() > 30:
        ...
        if n_gran > 0:
            ...
            out["granule_coarseness"] = _norm(mean_sigma, 1.0, 4.0)
        else:
        ...
        # inclusion (Dohle): locally bluer + paler than the bulk cytoplasm
        incl = (b_star < b_med - 1.2 * b_sd) & (Lc > L_med) & (satc < 0.5) & cyto
        incl = remove_small_objects(incl, min_size=15)
        incl_area = int(incl.sum())
        raw["inclusion_area"] = incl_area
        out["cytoplasmic_inclusion_visibility"] = _norm(
            incl_area / (seg["cyto_area"] + 1e-9), 0.005, 0.08)
...
def validate(df, out_dir, class_cols, n_fail, fail_reasons):
    ...
    for concept, targets in concept_targets.items():
        ...
        # reliability verdict: clearly beats VLM chance band (>0.60) AND varies
        entry["reliable"] = bool(best_disc >= 0.60 and entry["std"] > 0.03)
        if entry["reliable"]:
    ...
    with open(out_dir / "validation.md", "w") as f:
        f.write("\n".join(md) + "\n")
